...
//================================= SAME =====================================================
        ...
        $start = array(12, 14, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75, 80, 85,);
        $end = array(13, 19, 24, 29, 34, 39, 44, 49, 54, 59, 64, 69, 74, 79, 84, 101);

        $arrayResult = array();
        for ($i = 0; $i < 16; $i++) {
            $str = "SELECT Area, Gender, Count(ID) As CID "
                    . "FROM `" . $table . "` "
                    . "WHERE `temp`>= " . $start[$i] . " "
                    . "AND `temp` <= " . $end[$i] . " "
                    . "AND HH_Status = 'Head' "
            ...
            while ($saveAll = mysql_fetch_object($result)) {
                $arrayResult[$saveAll->Area][$i][$saveAll->Gender] = $saveAll->CID;
            }
        ...
        $r = 2;
        foreach ($arrayResult as $keyA => $valueA) {
            for ($z = 0; $z < 16; $z++) {
                echo $keyA . $start[$z] . " " . $end[$z] . " ";
                $c = 0;
                ...
                foreach ($valueA[$z] as $keyG => $valueG) {
                    echo $valueG . " ";
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($c, $r, $valueG);
                    $c++;
                }
                echo "<br>";
                $r++;
