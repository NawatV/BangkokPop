<html>
    <head>
        <meta charset="UTF-8">
        <title> Update Marital Status of Male for SimPhitlok </title>
    </head>
    <body>

        <?php
//-------------- Initiation (same but ActiveIndex = 1) -------------------------------------
        /** PHPExcel */
        ...
        $objWorksheet = $objPHPExcel->setActiveSheetIndex(1);  //select sheet
...
//----------------- AddMartialMale (similar) ---------------------------------------------
        ...
        /*
          ...
          echo $separated[$i][8]. " ";
          echo $unknow[$i][9]. " ";
          echo $unknowStatus[$i][10]. "<br>";
          }
          //echo "มากกว่า100 out : " .$endage[15][3]. "<br>";
         */
        ...
        for ($i = 0; $i <= 31; $i++) {  //query จำนวนตามกลุ่มอายุและaraa
            if ($endage[$i][3] == "มากกว่า100") {
            //-------- > 100 ----------------------------
                $number[$i] = mysql_query("SELECT count(*) as number from `" . $table . "` WHERE `Gender` = '" . $gender[$i][0] . "' AND `Area` = '" . $area[$i][1] . "' AND (`Age` >= " . $startage[$i][2] . " AND `Age` <= 100 OR `Age` = '" . $endage[$i][3] . "')");
                $groupage[$i] = mysql_fetch_assoc($number[$i]);
//                echo $i . " = " . $groupage[$i]['number'] . "<br>";
//                echo "test : gender" .$gender[15][0]. " area " .$area[15][1]. " sage " .$startage[15][2]. " end " .$endage[15][3]. "<br>"; 
            } else {
            //--------- OTHER CASES ---------------------
                $number[$i] = mysql_query("SELECT count(*) as number from `" . $table . "` WHERE `Gender` = '" . $gender[$i][0] . "' AND `Area` = '" . $area[$i][1] . "' AND `Age` >= '" . $startage[$i][2] . "' AND `Age` <= '" . $endage[$i][3] . "'");
                $groupage[$i] = mysql_fetch_assoc($number[$i]);
//                echo $i . " = " . $groupage[$i]['number'] . "<br>";
//                echo "test : gender" .$gender[15][0]. " area " .$area[15][1]. " sage " .$startage[15][2]. " end " .$endage[15][3]. "<br>"; 
            }
        ...
        for ($i = 0; $i <= 31; $i++) {   //คำนวณจำนวนคนแต่ละสถานะ
...
//------------- Update (similar) ---------------------------------------   
        ...
        for ($i = 0; $i <= 31; $i++) {
            for ($j = 4; $j <= 10; $j++) {
                if ($endage[$i][3] == "มากกว่า100") {
                    $female = "UPDATE `" . $table . "` SET `Marital_Status` = '" . $message[$j] . "' WHERE `Gender` = '" . $gender[$i][0] . "' AND `Area` = '" . $area[$i][1] . "' AND `Age` >= '" . $startage[$i][2] . "' AND (Age >= " . $startage[$i][2] . "  OR `Age` = '" . $endage[$i][3] . "') AND `Marital_Status` = '' ORDER BY RAND() LIMIT " . $status[$i][$j] . "";
                    mysql_query($female) or die(mysql_error());
                    echo "$i$j update successful <br>";
                } else {
                    $female = "UPDATE `" . $table . "` SET `Marital_Status` = '" . $message[$j] . "' WHERE `Gender` = '" . $gender[$i][0] . "' AND `Area` = '" . $area[$i][1] . "' AND `Age` >= '" . $startage[$i][2] . "' AND Age <= '" . $endage[$i][3] . "' AND `Marital_Status` = '' ORDER BY RAND() LIMIT " . $status[$i][$j] . "";
                    mysql_query($female) or die(mysql_error());
                    echo "$i$j update successful <br>";
                }
            }
